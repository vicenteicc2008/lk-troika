...
void Dmc_StartTimer(u32 uTsel)
{
	...
	case 0:
		...
		uTemp1 = Inp32(rTCON);
		uTemp1 = (uTemp1 & (~(0x1f))) | (uEnDz << 4) | (uAutoreload << 3) | (uEnInverter << 2) | (1 << 1) |
		         (0 << 0);
		/* set all zero in the Tiemr 0 con., Deadzone En or dis,
		 *      set autoload, set invert, manual uptate, timer stop */
		Outp32(rTCON, uTemp1); /*timer0 manual update*/
		uTemp1 = (uTemp1 & (~(0x1f))) | (uEnDz << 4) | (uAutoreload << 3) | (uEnInverter << 2) | (0 << 1) |
		         (1 << 0);
		/* set all zero in the Tiemr 0 con., Deadzone En or dis,
		 *      set autoload, set invert, manual uptate disable, timer start */
	...
	case 1:
		...
		/* set all zero in the Tiemr 0 con., Deadzone En or dis, set autoload,
		 *      set invert, manual uptate, timer stop */
		Outp32(rTCON, uTemp1); /*timer0 manual update*/
		uTemp1 = (uTemp1 & (~(0xf00))) | (uAutoreload << 11) | (uEnInverter << 10) | (0 << 9) | (1 << 8);
		/*		set all zero in the Tiemr 0 con., Deadzone En or dis, set autoload,
		 *      set invert,  manual uptate disable, timer start */
		Outp32(rTCON, uTemp1); /* timer0 start */
