...
extern int dsim_reset_panel(struct dsim_device *dsim);
extern int dsim_set_panel_power(struct dsim_device *dsim, u32 on);
...
void __dsim_dump(u32 id, struct dsim_regs *regs)
{
	...
	dsim_hex_dump(regs->regs, 0xFC);
	dsim_hex_dump(regs->regs + 0x200, 0x4);

	dsim_info("=== DSIM %d DPHY SFR DUMP ===\n", id);
	/* DPHY dump */
	/* PLL */
	dsim_hex_dump(regs->phy_regs + 0x0100, 0x24);
	/* MC */
	dsim_hex_dump(regs->phy_regs + 0x0300, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x03E0, 0x10);
	/* MD0 */
	dsim_hex_dump(regs->phy_regs + 0x0400, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x04C0, 0x20);
	/* MD1 */
	dsim_hex_dump(regs->phy_regs + 0x0500, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x05C0, 0x20);
	/* MD2 */
	dsim_hex_dump(regs->phy_regs + 0x0600, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x06C0, 0x20);
	/* MD3 */
	dsim_hex_dump(regs->phy_regs + 0x0700, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x07C0, 0x20);
