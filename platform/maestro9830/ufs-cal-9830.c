...
static struct ufs_cal_phy_cfg post_init_cfg_evt0_g4[] = {
	...
	{ 0x804, 0x06, PMD_ALL, PHY_PMA_TRSV, BRD_ALL },
	{ 0x808, 0x06, PMD_ALL, PHY_PMA_TRSV, BRD_ALL },
	{ 0x80C, 0x06, PMD_ALL, PHY_PMA_TRSV, BRD_ALL },
	{ 0x810, 0x01, PMD_ALL, PHY_PMA_TRSV, BRD_ALL },
	...
	{ 0xA14, 0x0A, PMD_ALL, PHY_PMA_TRSV, BRD_ALL },
	...
	for_each_phy_cfg(cfg) {
		for (i = 0; i < p->available_lane; i++) {
			...
			case PHY_PCS_TX_LR_PRD:
				ufs_lld_dme_set(hba,
				                UIC_ARG_MIB_SEL(cfg->addr,
				                                TX_LANE_0 + i),
				                (PCS_TX_LINE_RESET_PERIOD(p) >> 16) & 0xFF);
				ufs_lld_dme_set(hba,
				                UIC_ARG_MIB_SEL(cfg->addr + 1,
				                                TX_LANE_0 + i),
				                (PCS_TX_LINE_RESET_PERIOD(p) >> 8) & 0xFF);
				ufs_lld_dme_set(hba,
				                UIC_ARG_MIB_SEL(cfg->addr + 2,
				                                TX_LANE_0 + i),
