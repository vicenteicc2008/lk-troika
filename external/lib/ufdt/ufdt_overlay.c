		...
		struct ufdt *overlay_tree)
{
	...
	for_each_prop(it, overlay_fixups_node) {
		...
		phandle = ufdt_node_get_phandle(symbol_node);
		fixups_paths = ufdt_node_get_fdt_prop_data(fixups, &len);
		...
		struct ufdt_node *frag_node)
{
	...
	err = ufdt_overlay_node(target_node, overlay_node);
		...
		uint32_t phandle_offset)
{
	...
	if (prop_offsets_ptr == NULL || prop_data == NULL)
		return -1;
		...
		uint32_t phandle_offset)
{
	...
	int err = ufdt_local_fixup_node(overlay_node, local_fixups_node, phandle_offset);
		...
		struct ufdt *overlay_tree)
{
	...
	err = ufdt_overlay_do_local_fixups(overlay_tree, phandle_offset);
	if (err < 0) {
		...
		const struct ufdt *overlay_tree)
{
	...
	for (i = 0; i < overlay_tree->num_used_fdtps; i++) {
...
struct fdt_header *ufdt_apply_overlay(struct fdt_header *main_fdt_header,
		size_t main_fdt_size, void *overlay_fdtp, size_t overlay_size)
{
	size_t out_fdt_size;
	...
	err = ufdt_overlay_apply(main_tree, overlay_tree, overlay_size);
	if (err < 0)
		goto fail;
