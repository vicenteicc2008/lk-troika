		...
		const struct ufdt_prop_dict *dict, const char *name)
{
	...
	for (i = 0; i < size; i++) {
		...
		prop_name = fdt_string(dict->fdtp, fdt32_to_cpu(prop->nameoff));
		if (dto_strcmp(prop_name, name) == 0)
			return prop_ptr;
...
static int _ufdt_prop_dict_enlarge_if_needed(struct ufdt_prop_dict *dict)
{
	...
	for (i = 0; i < dict->mem_size; i++) {
		...
		if (prop == NULL)
			continue;
		new_prop_ptr = _ufdt_prop_dict_find_index(&temp_dict, prop);
