...
#if defined(CONFIG_EXYNOS_PANEL_CONNECT_CHECK)
int dsim_set_panel_detect(struct dsim_device *dsim)
{
	dsim_dbg("%s +\n", __func__);

	call_config_ops(dsim, set_gpio_panel_detect);
...
int dsim_check_panel_connect(struct dsim_device *dsim)
{
	...
	call_config_ops(dsim, check_panel_connect);
...
int dsim_probe(u32 dev_id)
{
	...
	dsim->res.ss_regs = (void __iomem *)EXYNOS9630_SYSREG_DPU;
...
#if defined(CONFIG_EXYNOS_PANEL_CONNECT_CHECK)
	dsim_set_panel_detect(dsim);
	dsim_check_panel_connect(dsim);
#endif
