	...
	u32 val)
{
	...
	if (!strcmp(name, "ssrx_sqhs_th")) {
		...
		if (get_chip_id_revision() > SOC_REVISION_EVT0) {
			...
			reg &= USBDP_TRSV_0407_LN1_TX_DRV_IDRV_IUP_CTRL_CLR_EVT1;
			reg |= USBDP_TRSV_0407_LN1_TX_DRV_IDRV_IUP_CTRL_EVT1(val);
			usbdp_cal_reg_wr(reg, regs_base + EXYNOS_USBDP_COM_TRSV_0407);
			reg = usbdp_cal_reg_rd(regs_base + EXYNOS_USBDP_COM_TRSV_0807);
			reg &= USBDP_TRSV_0807_LN3_TX_DRV_IDRV_IUP_CTRL_CLR_EVT1;
			reg |= USBDP_TRSV_0807_LN3_TX_DRV_IDRV_IUP_CTRL_EVT1(val);
...
void phy_exynos_usbdp_g2_run_lane(struct exynos_usbphy_info *info)
{
	...
	if (get_chip_id_revision() < SOC_REVISION_EVT1) {
...
void phy_exynos_usbdp_g2_enable(struct exynos_usbphy_info *info)
{
	...
	if (info->used_phy_port == 0) {
		...
		if (get_chip_id_revision() > SOC_REVISION_EVT0) {
			reg = 0;
			reg |= USBDP_CMN_A0288_DP_LANE_EN_SET(0xc); // usb rx0, tx1, dp tx2, tx3
		...
		reg = 0;
		reg |= USBDP_CMN_A0288_DP_LANE_EN_SET(0x3); // dp rx0, tx1, usb tx2, tx3
