...
bool node_name_eq(const struct ufdt_node *node, const char *name, int len)
{
	...
	if (!name)
		return false;
	if (dto_strncmp(name_of(node), name, len) != 0)
		return false;
	if (name_of(node)[len] != '\0')
		return false;
	return true;
}
...
int ufdt_node_add_child(struct ufdt_node *parent, struct ufdt_node *child)
{
	...
	if (!parent || !child)
		return -1;
	if (tag_of(parent) != FDT_BEGIN_NODE)
		return -1;
...
char *ufdt_node_get_fdt_prop_data(const struct ufdt_node *node, int *out_len)
{
	...
	prop = (struct fdt_property *)node->fdt_tag_ptr;
	if (out_len != NULL)
...
uint32_t ufdt_node_get_phandle(const struct ufdt_node *node)
{
	...
	ptr = ufdt_node_get_fdt_prop_data_by_name(node, "phandle", &len);
