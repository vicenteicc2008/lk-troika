...
#define L2_BANK_NUM			0x04
#define SET_END_L2			0x800
#define WAY_END_L2			0x8
#define OFFSET_END_L2			0x40
#define OFFSET_WIDTH_L2			0x06
...
#define L3CACHE_DUMP_ADDR		(u32)debug_snapshot_get_item_paddr("log_cachedump")
...
struct DL1RAMINDEX_LO_BIT_INFO {
	...
	unsigned int WAY:2;		//bit 20:19
	unsigned int SET_MSB:1;		//bit 21
	unsigned int DUMMY0:1;		//bit 22
...
struct L3TAG_BIT_INFO {
	u64 MOESI:3;			//bit 2:0
	u64 CPUID:4;			//bit 6:3
	u64 Inclusive:1;		//bit 7
	u64 NS:1;			//bit 8
	u64 PA_7_6:2;			//bit 10:9
	u64 PA_39_16:24;		//bit 34:11
	u64 ECC:7;			//bit 41:35
};
...
struct L2RAMINDEX_LO_BIT_INFO {
	...
	unsigned int L2PARTITION:2;	//bit 18:17
	unsigned int WAY:4;		//bit 22:19
	unsigned int DUMMY1:4;		//bit 26:23
	unsigned int RAMID:5;		//bit 31:27
};
...
struct L2TAG0_BIT_INFO {
	unsigned int MESI:3;		//bit 2:0
	unsigned int CP:1;		//bit 3
	unsigned int LOCK:1;		//bit 4
	unsigned int PA_8_6:3;		//bit 7:5
	unsigned int DUMMY:1;		//bit 8
	unsigned int PA_39_18:22;	//bit 30:9
	unsigned int NS:1;		//bit 31
};

struct L2TAG1_BIT_INFO {
	unsigned int LS_VA13_12:2;	//bit 33:32
	unsigned int res:31;
};
...
static void dump_l3_operation(u64 bank, u64 subbank, u64 set, u64 way)
{
	...
	l3tag_reg.ll_word = 0;
	if((way % L3_TAG_WAYS_CONCATENATE) == 0)
	    l3tag_reg.ll_word = ((l3_tag_raw_data1 & 0x3ff) << 32) | (l3_tag_raw_data0);
	else
	    l3tag_reg.ll_word = ((l3_tag_raw_data2 & 0xfffff) << 22) | (l3_tag_raw_data1 >> 10);

	/* determine writeback. modified or owned */
	if (!(((l3tag_reg.bits.MOESI & (MOESI_VALID | MOESI_MODIFIED)) ==
	     (MOESI_VALID | MOESI_MODIFIED)) && ((l3tag_reg.bits.NS) == 0x1)))
		return;

	l3addr_full = (((u64) l3tag_reg.bits.PA_39_16) << 16) |
		((u64) set << L3_SET_INDEX_ADDR_LSB) |
		((u64) l3tag_reg.bits.PA_7_6 << 6);

	dump_base = L3CACHE_DATA_DUMP_ADDR + bank * L3_SUBBANK_NUM * SET_END_L3 *
		WAY_END_L3*L3_DATA_SIZE;
	dump_base += subbank * SET_END_L3 * WAY_END_L3 * L3_DATA_SIZE;
	dump_base += set * WAY_END_L3 * L3_DATA_SIZE;
	dump_base += way * L3_DATA_SIZE;
		...
		for (subbank = 0; subbank < L3_SUBBANK_NUM; subbank++)
			for (set = 0; set < SET_END_L3; set++)
				for (way = 0; way < WAY_END_L3; way++)
					dump_l3_operation(bank, subbank, set, way);
}

static void dump_l2_operation(u64 core, u64 set_index, u64 way_index)
{
	union L2TAG0 l2tag0_reg;
	u64 l2addr_full;
	u64 dump_base = CORE_DUMP_ADDR + (CORE_DUMP_SIZE*core) +
			(set_index*WAY_END_L2 + way_index)*L2_LINE_RAW_DATA_SIZE;

	l2tag0_reg.word = readl(dump_base + 0x0);

	/* determine writeback. modified or owned */
	if (!(((l2tag0_reg.bits.MESI & (L2_MESI_VALID | L2_MESI_MODIFIED)) ==
	     (L2_MESI_VALID | L2_MESI_MODIFIED)) && ((l2tag0_reg.bits.NS) == 0x1)))
		return;

	l2addr_full = (((u64) l2tag0_reg.bits.PA_39_18) << 18) |
		((set_index/L2_BANK_NUM) << 9) |
		((u64) l2tag0_reg.bits.PA_8_6 << OFFSET_WIDTH_L2);
	...
	for (u32 offset = 0; offset < OFFSET_END_L2; offset += OFFSET_STEP_NUM_L2) {
		...
		printf("[L2] PA:0x%016llx: 0x%08x 0x%08x 0x%08x 0x%08x (set:0x%x, way:0x%x)\n",
				l2addr, l2data0, l2data1, l2data2, l2data3, set_index, way_index);
#endif
...
static void write_back_l2_cache(int core)
{
	...
	for (set = 0; set < SET_END_L2; set++)
		for (way = 0; way < WAY_END_L2; way++)
			dump_l2_operation(core, set, way);
}

static void write_back_d_cache_operation(u32 core, u64 set_index, u64 way_index)
{
	...
	dump_base = CORE_DUMP_ADDR + CORE_DUMP_SIZE * core + L2DATA_RAW_SIZE +
			(set_index * WAY_END_D + way_index) * DL1_LINE_RAW_DATA_SIZE;
	...
	daddr_full = (((u64)dl1tag0_reg.word & DL1TAG0_PA_39_12_MASK) << 12) |
		(((set_index & 0x3f) << OFFSET_WIDTH_D) & 0x0fff);
	...
	for (u32 offset = 0; offset < OFFSET_END_D; offset += OFFSET_STEP_NUM_D) {
		...
		printf("[DL1-%d] PA:0x%016llx: 0x%08x 0x%08x (set:0x%x, way:0x%x)\n",
				core, daddr, data0, data1, set_index, way_index);
#endif
