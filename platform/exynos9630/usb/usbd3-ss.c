...
#endif /* if defined(CONFIG_CPU_EXYNOS7885) */
...
static void exynos_usb_set_descriptor_tlb(void)
{
	...
	/* bus powered only. */
	oUsbDev3.m_oDesc.oDescConfig.bmAttributes = CONF_ATTR_DEFAULT | CONF_ATTR_SELFPOWERED;
...
static void exynos_usb_ep1_modify(void)
{
	...
	// should be reconfigured after ConnectDone event
	...
	if (!exynos_usb_set_ep_cfg(USBDEV3_EP_DIR_IN, 0, usbdev3_depcmdpar0_set_ep_cfg.data,
	                           usbdev3_depcmdpar1_set_ep_cfg.data))
		return;
}
...
static int exynos_usb_activate_ep0(void)
{
	...
	if (!exynos_usb_set_ep_cfg(USBDEV3_EP_DIR_OUT, 0, usbdev3_depcmdpar0_set_ep_cfg.data,
	                           usbdev3_depcmdpar1_set_ep_cfg.data))
		return 0;
...
static int exynos_usb_activate_ep(USBDEV3_EP_DIR_e eEpDir, u8 ucEpNum)
{
	...
	if (!exynos_usb_set_ep_cfg(eEpDir, ucEpNum, usbdev3_depcmdpar0_set_ep_cfg.data,
	                           usbdev3_depcmdpar1_set_ep_cfg.data))
		return 0;
...
static int exynos_usb_end_ep_xfer(USBDEV3_EP_DIR_e eEpDir, u8 ucEpNum, u32 uTri)
{
	...
	// wait time is longer than others'
	if (!exynos_usb_wait_ep_cmd_complete(eEpDir, ucEpNum, (10 * CMDCOMPLETEWAIT_UNIT)))
...
static int exynos_usb_prepare_1st_bulk_out_trb(void)
{
	...
	exynos_usb_fill_trb(g_pBulkOutTrb0, (u64)g_ucTempDownBuf, oUsbDev3.m_uBulkEPMaxPktSize, usbdev3_trb_ctrl.data,
	                    1);

	// . Issue Start Xfer for 1st Bulk Out Packet
	//------------------------------------
	if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_OUT, BULK_OUT_EP, (u64)g_pBulkOutTrb0, 0,
	                              &oUsbDev3.m_uTriOut[BULK_OUT_EP]))
		return 0;
	return 1;
}
...
void exynos_usb_handle_setup(void)
{
	...
	oUsbDev3.m_uDeviceRequestLength =
		(u32)((oUsbDev3.m_oDeviceRequest.wLength_H << 8) | oUsbDev3.m_oDeviceRequest.wLength_L);
	...
	case STANDARD_GET_DESCRIPTOR:
		...
		case DEVICE_DESCRIPTOR:
			...
			if (oUsbDev3.m_uDeviceRequestLength <= DEVICE_DESC_SIZE)
				exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescDevice)) + 0,
				                             oUsbDev3.m_uDeviceRequestLength);
			else
				exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescDevice)) + 0,
				                             DEVICE_DESC_SIZE);
		...
		case CONFIGURATION_DESCRIPTOR:
			...
			if (oUsbDev3.m_uDeviceRequestLength > CONFIG_DESC_SIZE) {
				...
				if (oUsbDev3.m_uDeviceRequestLength <= oUsbDev3.m_oDesc.oDescConfig.wTotalLengthL)
					exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig)) + 0,
					                             oUsbDev3.m_uDeviceRequestLength);
				else
					exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig)) + 0,
					                             oUsbDev3.m_oDesc.oDescConfig.wTotalLengthL);
			} else {                        // for win2k
				if (oUsbDev3.m_uDeviceRequestLength <= CONFIG_DESC_SIZE)
					exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig)) + 0,
					                             oUsbDev3.m_uDeviceRequestLength);
		...
		case ENDPOINT_DESCRIPTOR:
			...
			case 0:
				if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_SUPER)
					exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oSSDesc.oDescEp0),
					                             ENDPOINT_DESC_SIZE + ENDPOINT_COMP_DESC_SIZE);
				else
					exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oDesc.oDescEp0),
					                             ENDPOINT_DESC_SIZE);
				break;
			case 1:
				if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_SUPER)
					exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oSSDesc.oDescEp1),
					                             ENDPOINT_DESC_SIZE + ENDPOINT_COMP_DESC_SIZE);
				else
					exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oDesc.oDescEp1),
					                             ENDPOINT_DESC_SIZE);
				break;
			default:
				break;
	...
	case STANDARD_SET_FEATURE:
		...
		case TEST_MODE:
			if ((0 != oUsbDev3.m_oDeviceRequest.wIndex_L) ||
			    (0 != oUsbDev3.m_oDeviceRequest.bmRequestType)) {
				DBG_USBD3("\n %s(line %d)\n", __FILE__, __LINE__);
		...
		if (usRxCnt == 10) {            //Upload Request
			...
			if (usCheck == 0x1) {
				...
				if (oUsbDev3.m_uUploadSize > 0)	{
					...
					usCapTrbBufSiz = TRB_BUF_SIZ_LIMIT / oUsbDev3.m_uBulkEPMaxPktSize *
					...
					g_pBulkInTrbArray_Base = (usbdev3_trb_ptr_t)exynos_usb_malloc(
						g_uCntOfBulkInTrb * sizeof(usbdev3_trb_t),
						USBDEV3_MDWIDTH / 8);
					...
					for (i = 0; i < (g_uCntOfBulkInTrb - 1); i++) {
						exynos_usb_fill_trb(pBulkInTrb,
						                    (u64)(oUsbDev3.m_pUpPt + usCapTrbBufSiz * i),
						                    usCapTrbBufSiz,
					...
					//----------------------------
					if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_IN, BULK_IN_EP,
					                              (u64)g_pBulkInTrbArray_Base, 0,
					                              &oUsbDev3.m_uTriIn[BULK_IN_EP]))
						return;
				}
			} else {
				Assert(0);
			...
			if (oUsbDev3.m_uDownloadFileSize > usRxCnt) {     //there are more data to be received
				...
				g_pBulkOutTrbArray_Base = (usbdev3_trb_ptr_t)exynos_usb_malloc(
					g_uCntOfBulkOutTrb * sizeof(usbdev3_trb_t),
					USBDEV3_MDWIDTH / 8);
				...
				for (i = 0; i < (g_uCntOfBulkOutTrb - 1); i++) {
					exynos_usb_fill_trb(pBulkOutTrb,
					                    (u64)(oUsbDev3.m_pDownPt + usCapTrbBufSiz * i),
					                    usCapTrbBufSiz,
					                    usbdev3_trb_ctrl.data,
					                    1);
					pBulkOutTrb++;
				...
				uLastBufSize =
					((uLastBufSize + oUsbDev3.m_uBulkEPMaxPktSize - 1) /
					 oUsbDev3.m_uBulkEPMaxPktSize) *
					oUsbDev3.m_uBulkEPMaxPktSize;
				exynos_usb_fill_trb(pBulkOutTrb,
				                    (u64)(oUsbDev3.m_pDownPt + usCapTrbBufSiz * i),
				...
				//----------------------------
				if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_OUT, BULK_OUT_EP,
				                              (u64)g_pBulkOutTrbArray_Base, 0,
				                              &oUsbDev3.m_uTriOut[BULK_OUT_EP]))
					return;
...
static void exynos_usb_handle_event(void)
{
	...
	while ((uEventCount-- > 0) && (uLoop < USBDEV3_EVENT_BUFFER_COUNT)) {
		...
		uEventBufferCopied = *(oUsbDev3.m_pEventBuffer + oUsbDev3.m_CurrentEventPosition);
...
int exynos_usbctl_init(void)
{
	...
	// to keep the core in reset state until phy clocks are stable(GCTL??11ë²?bit ?¤ëª… ì°¸ì¡°)
	...
	usbdev3_gctl.data = readl(rGCTL);
	// to keep the core out of reset state after phy clocks are stable(GCTL??11ë²?bit ?¤ëª… ì°¸ì¡°)
